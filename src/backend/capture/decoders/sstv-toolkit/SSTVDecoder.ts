...
export class SSTVDecoder {
  ...
  private lastVisParityOk = false
  private visFreqShift = 0
  private currentChromaType: 'U' | 'V' = 'V'

  constructor(sampleRate = 48000, options: SSTVDecoderOptions = {}) {
  ...
  private get currentMode(): SSTVMode {
    if (this.mode === null) throw new Error('SSTV mode not detected')
    return this.mode
  }
  ...
  decodeVIS(samples: Float32Array, startIdx: number, freqShift = 0): number {
    let idx = startIdx
    let visCode = 0
    let ones = 0
